...
public class DList<T extends Comparable<T>> implements IList<T>, Cloneable{
	...
	public DList<T> clone() {
		//TODO: Complete
		DList<T> clone = new DList<T>();
		Node<T> currentNodeElem = this.header;
		Node<T> currentCloneElem = clone.header;
		
		while(currentNodeElem.getNext() != null) {
			currentCloneElem.setNext(currentNodeElem.getNext());
			
			currentCloneElem = currentCloneElem.getNext();
			currentNodeElem = currentCloneElem.getNext();
		}
	...
	public T remove(Node<T> elem) {
		...
		while(currentNodeElem.getNext() != null) {
			if(currentNodeElem.getElement() == elem.getElement()) {
				currentNodeElem.getPrev().setNext(currentNodeElem.getNext());
				currentNodeElem.getNext().setPrev(currentNodeElem.getPrev());
				
				currentNodeElem.setNext(null);
				currentNodeElem.setPrev(null);
				size--;
				return elem.getElement();
			}
			
			currentNodeElem = currentNodeElem.getNext();
		}
